...
HEPCLASS_IMPL_FUNCX_MORE_S(BegnHand, ProcessOne)
HEPCLASS_IMPL_FUNCX_MORE_S(BegnHand, DisplayMsg)
HEPCLASS_IMPL_FUNCX_MORE_S(BegnHand, on_CMD_KEEPALIVE_REQ)
...
{
	//setToCache(s_scommid_key, std::to_string(ss_svrid_gen));
	return 0;
}
...
int BegnHand::on_CMD_WHOAMI_REQ( IOHand* iohand, const Value* doc, unsigned seqid )
{
	...
	if (first)
	{
		...
		// svrid属性新分配要设置
		CliMgr::Instance()->setProperty(iohand, CONNTERID_KEY, std::to_string(svrid));
